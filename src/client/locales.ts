...
export const zh: Record<MultiRootKey, string> = {
  ...
  'panel.add.pathPlaceholder': '文件夹绝对路径，例如 /home/user/projects',
  'panel.add.namePlaceholder': '别名（可选，默认取文件夹名）',
  'panel.add.browse': '浏览',
  'panel.add.submit': '添加',
  'panel.add.hint': '只能挂载存在的目录，可先浏览选择（Windows 上从盘符开始）。',
...
export const en: Record<MultiRootKey, string> = {
  ...
  'panel.add.pathPlaceholder': 'Absolute folder path, e.g. /home/user/projects',
  'panel.add.namePlaceholder': 'Alias (optional; defaults to the folder name)',
  'panel.add.browse': 'Browse',
  'panel.add.submit': 'Add',
  'panel.add.hint': 'Only existing directories can be attached; browse to select (drive level first on Windows).',
